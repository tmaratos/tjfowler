...
(function () {
  ...
  function showDeniedDebug(info) {
    ...
    if (errorEl) errorEl.textContent = info?.error || "(none)";
    showView("denied");
  }
  ...
  async function checkAdminAccess() {
    ...
    if (authError) {
      console.error("[admin] getUser:", authError.message);
      denied.error = authError.message;
      return denied;
    }
    ...
    denied.email = email || null;
    if (!email) {
      denied.error = "Authenticated user has no email";
      return denied;
    }

    let { data, error } = await sb.from("admin_users").select("*").ilike("email", email).maybeSingle();
    if (error) {
      console.error("[admin] admin_users email lookup:", error.message);
      denied.error = error.message;
      return denied;
    }
    if (data) {
      denied.rowFound = true;
      if (!adminRowAllowed(data)) {
        denied.error = `Role is "${data.role}", expected "admin"`;
        return denied;
      }
      denied.ok = true;
      return denied;
    ...
    ({ data, error } = await sb.from("admin_users").select("*").eq("user_id", user.id).maybeSingle());
    if (error) {
      console.error("[admin] admin_users user_id lookup:", error.message);
      denied.error = error.message;
      return denied;
    }
    if (data) {
    ...
    denied.error = "No matching row in admin_users (email or user_id)";
    return denied;
  }
  ...
  async function requireSession() {
    ...
    if (authError) {
      console.error("[admin] getUser:", authError.message);
      showAlert(loginAlert, authError.message, "error");
      showView("login");
      return false;
    }
